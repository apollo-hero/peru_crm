    ...
    public function facebookViaAPI(Request $request) { 
        ...
        try{
            ...
            if($AuthUser){
                ...
                $nombre=$name[0];
                $AuthUser->first_name=$nombre;
                $apellidos=isset($name[1]) ? $name[1] : '';
                $AuthUser->last_name= $apellidos;

                if($FacebookDrive->email!=""){
                    $AuthUser->email=$FacebookDrive->email;
                }
                else{
                    $AuthUser->email=$nombre.$apellidos.'@notienecorreoesteface.com';
                }
			...
			if(config('constants.referral', 0) == 1) {
				...
				if($request->referral_unique_id != null){
					...
					(new ReferralResource)->create_referral($request->referral_unique_id,$AuthUser);                
				}
            }
