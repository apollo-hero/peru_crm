...
class AdminController extends Controller
{
	...
	public function recarga()
	{
		...
		//return view('admin.recargarbilletera.blade');
		$users = DB::table('providers')
		->join('provider_wallet', 'providers.id', '=', 'provider_wallet.provider_id')
		->select('provider_wallet.*', 'providers.first_name', 'providers.mobile','providers.last_name')
		->get();
		$ProviderWallet=$users;
        return view('admin.recarga',compact('ProviderWallet'));
	}
	public function debito()
	{
		...
		->get();
		$ProviderWallet=$users;
        return view('admin.debito',compact('ProviderWallet'));
	}
	
	public function billeterarecargar(Request $request){
	
	  
		$provider=Provider::where('mobile',$request->phone)->first();
	   
		$TablaProvideWallet=ProviderWallet::orderBy('transaction_id','desc')->first();
	
 
		 $providerWallet=new ProviderWallet;        
		 $providerWallet->provider_id=$provider->id;        
		     
		 $providerWallet->transaction_id=$TablaProvideWallet->transaction_id+1;        

		$numeroGenerado=$TablaProvideWallet->transaction_id+1;
		$providerWallet->transaction_alias="PDL".$numeroGenerado."";        
		 $providerWallet->transaction_desc='RECARGA WEB'; 
		 $providerWallet->type='C';
		 
		 $providerWallet->amount=$request->cantidad;
		...
		if($providerWallet->payment_mode==null){
		 ...
		 if(empty($provider->wallet_balance))
			 $providerWallet->close_balance=$request->cantidad;
		 else            
			 $providerWallet->close_balance=$provider->wallet_balance+($request->cantidad);        
 
		 $providerWallet->save();
 
		
		 $provider->wallet_balance=$provider->wallet_balance+($request->cantidad);
		 $provider->status="approved";
		 $provider->save();
 
		 //DB::table('provider_services')->where('provider_id',$request['id'])->where('status','hold')->update(['status' =>'active']);
		 DB::table('provider_services')->where('provider_id',$provider->id)->update(['status' =>'active']);
		 
		 return back();
	 }

	
	 public function billeteradebitar(Request $request){
		...
		$numeroGenerado=$TablaProvideWallet->transaction_id+1;
		$providerWallet->transaction_alias="PDL".$numeroGenerado."";        
		 $providerWallet->transaction_desc=$request->motivo; 
	...
	public function godseye_list(Request $request)
	{
	   try {
			$filter_text = "";
			if($request->filter_text != null) $filter_text = $request->filter_text;
			if($request->status == 'STARTED' || $request->status == 'ARRIVED' || $request->status == 'PICKEDUP') {
				...
				})->select('id', 'first_name', 'last_name', 'mobile', 'email', 'latitude', 'longitude')
				->where('last_name', 'like', '%'.$filter_text.'%')->get();
				...
				})->select('id', 'first_name', 'last_name', 'mobile', 'email', 'avatar', 'status', 'latitude', 'longitude')
				->where('last_name', 'like', '%'.$filter_text.'%')->get();
			}
		...
		if (!file_exists($config)) {
			...
			$data = "<?php 
				...
				return array(
					...
					'environment' => '',
					'ios_user_push_password' => '',
					'ios_provider_push_password' => '',
