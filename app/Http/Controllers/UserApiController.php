...
class UserApiController extends Controller
{
	...
	public function signup(Request $request)
	{
		...
		$this->validate($request, [
			...
			if(config('constants.referral', 0) == 1) {
				...
				if($request->referral_unique_id != null){
					
					//call referral function
					(new ReferralResource)->create_referral($request->referral_unique_id,$User);                
				}
			}  
	...
	public function test_firebase() {
		...
		$Providers = Provider::with('service')
			->select('id','status')->whereIN('id', $ans)
	...
	public function send_request(Request $request) {
		...
		$Providers = Provider::with('service')
			...
			->orderBy('distance','asc')
			->get();
		//	$this->firebaseService->save_data("service_type",$service_type .' Nombre:'. Auth::user()->first_name .' '.Auth::user()->last_name . ' '. Auth::user()->mobile);
			//$this->firebaseService->save_data("service_type",$service_type);
		...
		//	$this->firebaseService->save_data("service_type",$service_type);
		//	$this->firebaseService->save_data("ans",$ans, true);
		//	$this->firebaseService->save_data("rad",$distance, false);
		...
		if(count($Providers) == 0) {
			if($request->ajax() || $request->wantsJson()) {
				...
				return back()->with('flash_success', trans('api.ride.request_scheduled'));
			}
		}		
		...
		try{
			...
			$UserRequest->referencia = $request->referencia;
			$UserRequest->tiposervicio = $tipo_servicio->name;
		...
		//	$UserRequest->distance = $request->distancia;

			/*$apiurl = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$request->s_latitude.",".$request->s_longitude."&destinations=".$request->d_latitude.",".$request->d_longitude ."&mode=driving&sensor=false&key=".config('constants.map_key');
			...
			if(!empty($location['rows'][0]['elements'][0]['status']) && $location['rows'][0]['elements'][0]['status']=='OK'){
				...
				$UserRequest->distance = $location['rows'][0]['elements'][0]['distance']['value'];
