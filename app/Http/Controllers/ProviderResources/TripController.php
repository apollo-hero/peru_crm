...
class TripController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		/*	if($request->ajax() || $request->wantsJson()) {
			...
			$Response = [
					...
					'referral_text' => "<p style='font-size:16px; color: #fff;'>Invita a tus amigos<br> y gana <span color='#00E4C5'>".config('constants.currency', '')."".config('constants.referral_amount', '0')."</span>por Cada uno</p>",
					'referral_total_count' => $referral_total_count,
					'referral_total_amount' => $referral_total_amount,
					'referral_total_text' => "<p style='font-size:16px; color: #000;'>Importe de Referidos: ".$referral_total_amount."<br>Cantidad de Referidos:".$referral_total_count."</p>",
				];
			//	return 'SI ENTRO 005';
			...
			//return $AfterAssignProvider;
			//return $BeforeAssignProvider;
	        //return $IncomingRequests;
			return $Response;
		...
		try{
			...
			if($request->ajax() || $request->wantsJson()) {
				...
				$Response = [
						...
						'referral_total_count' => $referral_total_count,
						'referral_total_amount' => $referral_total_amount,
						'referral_total_text' => "<p style='font-size:16px; color: #000;'>Importe Referidos: ".$referral_total_amount."<br>Cantidad Referidos:".$referral_total_count."</p>",
					];
	...
	public function rate(Request $request, $id)
	{
		...
		try {
			...
			if($UserRequest->Enviada_despachador=='NO'){
				...
				if($UserRequest->rating == null) {
					UserRequestRating::create([
							...
							'provider_comment' => $request->comment,
							'user_rating' => 5,
							'user_comment' => 'Desde Despachador',
						]);
				} else {
					$UserRequest->rating->update([
							'provider_rating' => $request->rating,
			...
			//RequestFilter::where('request_id', $id)->delete();
				...
				RequestFilter::where('request_id' , $id)
...
*/
			...
			//	}
				...
				RequestFilter::where('request_id', $UserRequest->id)->update(['status' =>'1']);
			//}
	...
	public function update(Request $request, $id)
	{
		...
		//$this->firebaseService->save_data($request, $id);
		...
		try{
			...
			if($request->status == 'PICKEDUP'){
				...
				$Descuento = $UserRequest->promocode_id;
				if($UserRequest->promocode_id>0){
					...
					if($Promocode = Promocode::find($UserRequest->promocode_id)){
						...
						$discount_amount = ($discount_per/100);
	
						if($discount_amount>$Promocode->max_amount){
							$Descuento = $Promocode->max_amount;
							//$Descuento= 11;
						}
						else{
						...
						$UserRequest->descuentopromocion = $UserRequest->preciocentral *  $Descuento;
						$UserRequest->preciocentral =  $UserRequest->preciocentral - ($UserRequest->preciocentral *  $Descuento);
			...
			if($request->status == 'DROPPED') {
				...
				if($UserRequest->is_track == "YES"){
					...
					if($meters >= 1000){
						...
						//$UserRequest->route_key = $UserRequest->destination_log;
					    $destino=$UserRequest->destination_log;
				...
				$UserRequest->invoice = $this->invoice($id, ($request->toll_price != null) ? $request->toll_price : 0,$destino);
				...
				if(config('constants.referral_amount')>0){
					...
					$usuarioLogueado=$UserRequest->user_id;
			
					$Listadoreferido=ReferralHistroy::where('referral_id',$usuarioLogueado)->first();
					
					$idnuevito=$Listadoreferido->referrer_id;
					
				
					$referral_id=$idnuevito;
					...
					if(!empty($referral)){
						if(config('constants.referral_amount')>0){
							...
							$ReferralHistroy=ReferralHistroy::select(DB::raw("group_concat(id) as ids"))->where('referrer_id',$referral_id)->where('type',$type)->where('status','P')->groupBy('referrer_id')->get();
							...
							if(config('constants.referral_count')>0 && config('constants.referral_count') == $Referralcount) {                
								...
								(new TripController)->referralCreditDebit(config('constants.referral_amount'),$referral_id,$type);
			
								//update histroy table process status to complete
								ReferralHistroy::where('referrer_id',$referral_id)->where('type',$type)->where('status','P')->update(['status' => 'C']);
			...
			//$this->firebaseService->save_data('ModelNotFoundException',$e);
			return response()->json(['error' => trans('api.unable_accept')]);
			
		} catch (Exception $e) {
			//$this->firebaseService->save_data('Exception',$e);
			return response()->json(['error' => trans('api.connection_err')]);
		}
	}
  ...
  public function test(Request $request)
	...
	public function invoice($request_id, $uptoll_price = 0,$destino='')
	{

		try {                      
			//$this->firebaseService->save_data($request_id, 'enter in function invoice');
			//$this->firebaseService->save_data('request_id from function invoice= '.$request_id,$request_id);
			$UserRequest = UserRequests::with('provider')->with('service_type')->findOrFail($request_id);  
			if(empty($UserRequest) || $UserRequest == null) {
				$this->firebaseService->save_data($request_id, 'WRONG');
				return;
			}
			else{
				//$this->firebaseService->save_data($request_id, 'RIHGT');
			}
			...
			$Precio=0;// creado for Augus 17/08/2019

			//added the common function for calculate the price
			if(empty($UserRequest->travel_time) || $UserRequest->travel_time == null) $UserRequest->travel_time = -1;
			if(empty($UserRequest->distance) || $UserRequest->distance == null)$UserRequest->distance = -1;
			if(empty($UserRequest->service_type_id) || $UserRequest->service_type_id == null) $UserRequest->service_type_id = -1;
			if(empty($destino) || $destino == null) $destino = -1;
			$requestarr = array(
			...
			if(!empty($pricedata)){
			//	$this->firebaseService->save_data("data no empty", $pricedata, true);
				...
				//$this->firebaseService->save_data("$calculator", $calculator);
			}
			else{
			//	$this->firebaseService->save_data("data  empty ...", "data  empty ...");
			...
			//$Total = $Distance + $Tax; // comentaado por Augus 17/08/2019
			//$payable_amount = $Distance + $Tax - $Discount; // comentaado por Augus 17/08/2019
			//$Total = $Distance;
			$Total=$UserRequest->preciocentral;
			...
			if(empty($toll_price)) $toll_price = 0;
			...
			*/ 
			...
			$Payment->fixed = $Fixed  + $Surge +$peakamount;
			//$this->firebaseService->save_data("Payment->fixed", $Payment->fixed);
			...
			if($UserRequest->preciocentral>0){
				$Payment->provider_pay = $UserRequest->preciocentral - $Discount;
			}
			else{
				$Payment->provider_pay = $ProviderPay - $Discount;
			}

			//$Payment->total = $Payment->provider_pay;
			...
			if($UserRequest->use_wallet == 1 && $payable_amount > 0){
				...
				if($Wallet != 0){
					...
					if($payable_amount > $Wallet) {
						...
						$Payment->payable = $Payment->fixed-$Payment->wallet;
						//$Payment->payable = "13.00";
						...
						//$UserRequest->preciocentral = $UserRequest->preciocentral - $Discount;
						//$UserRequest->descuentopromocion = $Discount;
						$UserRequest->save();
				...
				if($UserRequest->payment_mode == 'CASH'){
					...
					if($UserRequest->preciocentral>0){
						$Payment->total = $UserRequest->preciocentral - $Discount;
					}
					else{
						$Payment->total = $Payment->provider_pay - $Discount;
					}
					...
					if($UserRequest->preciocentral>0){
						$Payment->payable = $UserRequest->preciocentral - $Discount;
					}
					else{
						$Payment->payable = $Payment->provider_pay - $Discount;
					}

					$Payment->fixed = $Payment->provider_pay;
					$Payment->cash = $Payment->provider_pay;
					
			
					/*	$Payment->round_of = round($Precio)-abs($Precio);
					$Payment->total = 123;
					$Payment->payable = 231;*/
				}
				else{
	...
	public function summary(Request $request)
	{
		...
		try{
			if($request->ajax() || $request->wantsJson()) {
				...
				/*$rides = UserRequests::where('provider_id', '344')->count();
				$revenue = UserRequestPayment::where('provider_id', '344')->sum('provider_pay');
				$cancel_rides = UserRequests::where('status','CANCELLED')->where('provider_id','344')->count();
				$scheduled_rides = UserRequests::where('status','SCHEDULED')->where('provider_id','344')->count();
*/
				//return null;
				$from = date($request->desde)." 00:00:00";

				$to = date($request->hasta)." 23:59:59";

				$rides = UserRequests::where('provider_id',Auth::user()->id)->whereBetween('created_at',[$from, $to])->count();
				


				$revenue = UserRequestPayment::where('provider_id', Auth::user()->id)->whereBetween('created_at',[$from, $to])->sum('provider_pay');
				//$revenue=$revenue->whereBetween('created_at',[$from, $to]);

				$cancel_rides = UserRequests::where('status','CANCELLED')->where('provider_id',Auth::user()->id)->whereBetween('created_at',[$from, $to])->count();
				//$cancel_rides=$cancel_rides->whereBetween('created_at',[$from, $to]);


				$scheduled_rides = UserRequests::where('status','SCHEDULED')->where('provider_id',Auth::user()->id)->whereBetween('created_at',[$from, $to])->count();
				//$scheduled_rides=$scheduled_rides->whereBetween('created_at',[$from, $to]);
	...
	*/ 
	...
	protected function createProviderWalletRecargas(Request $request, $id){
		...
		$providerWallet->type=$request['type'];
		$providerWallet->amount=$request['amount'];
	   //	$providerWallet->payment_mode=$request['payment_mode'];
	   if($providerWallet->payment_mode==null){
		$providerWallet->payment_mode="PAYUMONEY";
	   }
	   else{
		$providerWallet->payment_mode=$request['payment_mode'];
	   }
		...
		//DB::table('provider_services')->where('provider_id',$request['id'])->where('status','hold')->update(['status' =>'active']);
		DB::table('provider_services')->where('provider_id',$request['id'])->update(['status' =>'active']);
	...
	protected function createProviderWalletRecargasDesdeApp(Request $request, $id){
		
	

		//$provider=Provider::findOrFail($request['id']);
		$provider=Provider::where('mobile',$request['id'])->first();
