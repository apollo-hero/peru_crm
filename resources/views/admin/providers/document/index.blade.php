          ...
          @if(Setting::get('demo_mode', 0) == 1)
            ...
            <div class="row">
                ...
                <form action="{{ route('admin.provider.document.store', $Provider->id) }}" method="POST">
                    ...
                    <input type="input" name="is_create" style="display:none;" value="0" id="service_is_create"/>
                    @if( Setting::get('demo_mode', 0) == 0)
                    <div class="col-xs-3">
                        @can('provider-service-update')<button class="btn btn-primary btn-block" type="submit">@lang('admin.update')</button>@endcan
                        @can('provider-service-update')<button class="btn btn-success btn-block" type="submit" id="service_create_button">Create</button>@endcan
                    </div>
                    @endif
                </form>
            </div>
...
<script type="text/javascript">
    ...
    $('#service_create_button').click(function() {
        $('#service_is_create').val('1');
    });
</script>
@endsection